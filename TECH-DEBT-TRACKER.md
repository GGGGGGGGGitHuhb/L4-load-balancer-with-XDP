...
- `docs/specs/config-schema.md`、`docs/specs/tcp-forwarding-semantics.md` 与 `docs/runbooks/local-dev-env.md` 已存在；benchmark方法、模板与工具验证样本已于V0.4/S1交付，正式用户态性能报告及公开原始包已于S3独立验收。
...
V0.4-S3-D1保持Approved，S3及V0.4开发范围Completed（2026-09-10）。Builder001、Reviewer001独立PASS和Leader003齐备，六AC/六版本标准满足。独立Debug28/28、Release29/29、Production/uid38/F-001/11短调用通过；Builder和Reviewer各自48run独立原始数学及公开全表核对通过，全部PID/fd回收。历史工具元数据修复批次保留，未拼接。TD-004正式用户态报告/版本矩阵义务已交付并验收，XDP义务保持；无新增发现、返工、技术债、阻塞或PM决策。本阶段尚未提交、合并或发布，由父按授权本地提交；不表示V1.0。
...
Completed（2026-09-09），V0.3-S3-D1保持Approved；Builder001、Reviewer001独立PASS、Leader003收尾齐备，S3与V0.3开发范围完成，六条标准见 `docs/specs/v0.3-acceptance.md`。独立Debug22/22、Release23/23、Production/38CLI、重复/并行/工具负向和10次ICMP采样通过，29个产品PID回收且fixture fd相等；历史ICMP根因未定位、不声称已修复。TD-004 V0.3健康/指标/公共运行手册及版本矩阵义务均完成，未来XDP/benchmark保留。偏差影响、新债、未解决发现、阻塞、PM决策：None；父按授权本地提交。

### V0.3 / S2 指标模型与输出

Completed（2026-09-09），V0.3-S2-D1保持Approved；Builder002、Reviewer002独立PASS及Leader003收尾，见 `docs/leader/reports/V0.3/S2-report-003.md`。R-S2-01/02范围内返工已关闭，无带债验收；最终Debug19/19、Release20/20、Production及五负向通过。TD-004 metrics规格义务已交付并独立验收，后续XDP/benchmark继续；S3准备中。S2已合并PR8并标记v0.3-s2（2f5c26d，含dfcffe6）；当前codex/v0.3-s3准备S3。重大偏差、新增技术债、阻塞、PM决策：None。

### V0.3 / S1 健康检查状态机

Completed（2026-09-09），V0.3-S1-D1保持Approved；Builder001、Reviewer001独立PASS、Leader003收尾完成，见 `docs/leader/reports/V0.3/S1-report-003.md`。Debug14/14、Release15/15及Production验证通过，42文件指纹一致。S1已合并PR7并标记v0.3-s1（9971818b），含实现0780b7d；当前分支codex/v0.3-s2准备S2。TD-004健康规格义务已履行，指标规格已于S2交付并独立验收；长期债项不整体关闭。默认off、Unknown预热与UDP代理信号边界保持；重大偏差、新增技术债、阻塞和PM决策：None。

### V0.2 / S3 UDP 验证与语义文档

状态：Completed（2026-09-09），V0.2-S3-D1保持Approved，Reviewer001 PASS、Leader003收尾。S3七条AC及V0.2六条标准均满足，见 `docs/specs/v0.2-acceptance.md` 和 `docs/leader/reports/V0.2/S3-report-003.md`。独立Debug快速10/10、Release11/11及原60秒产品验证通过。无新增债项、返工、未解决发现或PM待决策；S3已合并，远端main与v0.2-s3标签核验为7821b25；当前V0.3/S1已Completed并合并/标记，codex/v0.3-s2准备S2。
...
2026-09-22 路线调整：后续产品验证明确分为S2真实UDP转发、S3运行期发布/故障摘除与恢复、S4性能对比。S3须用持续真实流量核验配置一致性、更新失败及健康变化，S4在功能验收后执行。下列较早记录中的“S3性能”保留历史语境，当前以本段及ROADMAP为准。
...
以下为截至 2026-09-10 的历史背景；其中云端强制路线已被上述当前决定替代：
...
2026-09-22 路线调整：S3补充运行期配置发布、与UDP服务匹配的探活契约、全不可用及流映射变化规格与运行手册；原性能对比/边界总结移至S4。阶段详细设计再确定ABI演进方式，不允许直接向S1/S2已freeze的配置map写入来冒充热更新。下列较早“S3性能”记录为历史状态。
...
2026-09-14：V1.1/S1新增公开 `docs/runbooks/xdp-build.md` 并经Reviewer001核对构建命令/依赖/测试一致，S1构建文档义务完成。项目加载运行说明、map schema及后续数据面/性能文档继续按S2/S3和V1.2推进，不整体关闭。
...
- `V0.4/S1` 已完成并独立验收 `docs/benchmarks/methodology.md`、`report-template.md`及工具验证样本；S3正式用户态性能报告、原始包与版本矩阵已交付验收，XDP义务保持。
...
方法与工具已按V0.4/S1路线交付并独立验收；正式用户态报告已在S3交付验收；后续平台/负载归因仍需对应验证，不扩大现有结论。
...
V0.1与V0.2开发范围已完成；V0.3/S1 Completed且已合并/标记，S2 Completed且合并/标记、S3 Completed，V0.3开发范围完成。以下 S1 启动检查点已完成并经阶段验收确认，仅保留作为历史记录：
