...
int bpf_map_update_elem(int fd, const void *key, const void *value,
                        uint64_t flags) {
  if ((faultMatches(fd, "backend-write", "l4lb_be_v2") &&
       *(const uint32_t *)key == 1) ||
      faultMatches(fd, "config-write", "l4lb_cfg_v2")) {
    errno = EIO;
    return -1;
  }
...
int bpf_map_lookup_elem(int fd, const void *key, void *value) {
  if (faultMatches(fd, "stats-read", "l4lb_stats_v2") ||
      faultMatches(fd, "config-read-error", "l4lb_cfg_v2")) {
    errno = EIO;
    return -1;
  }
  ...
  if (!result && (faultMatches(fd, "config-mismatch", "l4lb_cfg_v2") ||
                  (faultMatches(fd, "backend-mismatch", "l4lb_be_v2") &&
...
int bpf_map_freeze(int fd) {
  if (faultMatches(fd, "backend-freeze", "l4lb_be_v2") ||
      faultMatches(fd, "config-freeze", "l4lb_cfg_v2")) {
    errno = EIO;
    return -1;
  }
